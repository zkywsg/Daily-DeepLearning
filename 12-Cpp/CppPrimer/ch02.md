...
- 允许把变量声明为constexpr类型/方便编译器来验证变量的值是否是常量表达式

```cpp
constexpr int sz = size()//只有size是一个constexpr函数的时候/才是一条正确的声明语句
```

- constexpr会把它定义的对象置为顶层const
...
constexpr const int *p = &i; //怕是常量指针,指向整型常量i
...
// decltype的结果可以是引用类型
int i = 42.*p = &i,&r = i;
decltype(r+0) b; // b is int 
decltype(*p) c; // 错误,c是int& ,必须初始化


// decltype的表达式如果加上括号的变量/结果将是引用
